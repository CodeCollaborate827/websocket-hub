...
public class KafkaProducerImpl implements KafkaProducer {
  ...
  @Override
  public void sendSessionEvent(Session sessionEvent) {
    try {
      ...
      if (result.isFailure()) {
        log.error("Send {} event failed", sessionEvent.getStatus());
      } else {
        log.info("Send {} event success", sessionEvent.getStatus());
      }
    } catch (JsonProcessingException e) {
      throw new RuntimeException(
          "Failed to process JSON for " + sessionEvent.getStatus() + " event", e);
    }
  }
}
